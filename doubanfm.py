#!/usr/bin/python3
"""A douban.fm cilent for rhythmbox"""
...
class DoubanfmEntryType(RB.RhythmDBEntryType):
    """Entry type of rhythmbox for doubanfm plugin"""
    def __init__(self):
...
class DoubanfmPlugin(GObject.Object, Peas.Activatable):
    """douban.fm plugin for rhythmbox"""
...
class DoubanfmSource(RB.StreamingSource):
    """A rhythmbox `soure' for doubanfm plugin"""
    def __init__(self, **kwargs):
        ...
        self.container_types = {
            'user': {
                'attributes': [
                    'username', 'kind', 'uri', 'permalink_url', 'avatar_url',
                    'description', 'cache_entry'],
                'tracks-url': '/tracks.json',
                'tracks-type': 'plain',
            },
            'playlist': {
                'attributes': ['title', 'kind', 'uri', 'permalink_url',
                               'artwork_url', 'description', 'cache_entry'],
                'tracks-url': '.json',
                'tracks-type': 'playlist',
            },
            'group': {
                'attributes': ['name', 'kind', 'uri', 'permalink_url',
                               'artwork_url', 'description', 'cache_entry'],
                'tracks-url': '/tracks.json',
                'tracks-type': 'plain',
            },
    ...
    def new_model(self):
        ...
        db.query_append_params(q, RB.RhythmDBQueryType.EQUALS,
                               RB.RhythmDBPropType.TYPE, plugin.entry_type)
        db.query_append_params(q, RB.RhythmDBQueryType.EQUALS,
    ...
    def add_track(self, db, entry_type, item):
        ...
        if entry:
            ...
            home_page = 'https://douban.fm/song/%sg%s' % (item['sid'],
                                                          item['ssid'])
            ...
            db.entry_set(entry, RB.RhythmDBPropType.LAST_SEEN, self.search_count)
            db.entry_set(entry, RB.RhythmDBPropType.BEATS_PER_MINUTE,
                         int(item.get('kbps', '128')))
    ...
    def rb_loader_cb(self, data):
        artists = []
        channels = []
        songs = []
        songlists = []
        search_results = json.loads(data.decode())
        ...
        for songlist in songlists:
            ...
            self.loader.get_url(url, self.rb_loader_cb)

        self.doubanfm_cache['song'] = songs
        self.search_tracks_api_cb(songs)
        
        print("\DEBUG### ok NUmber of entry of self.songs is: %d" % len(self.props.base_query_model))
        shell = self.props.shell
        for treerow in self.props.base_query_model:
            entry, path = list(treerow)
            print(entry.get_string(RB.RhythmDBPropType.TITLE),
                  entry.get_string(RB.RhythmDBPropType.MOUNTPOINT))

        entry = self.songs.get_playing_entry()
        print('entry=%s' % str(entry))

    def rb_loader_do_search(self):
    ...
    def search_entry_cb(self, widget, term):
        ...
        self.new_model()  # clear the songs entries
        self.rb_loader_do_search()
    ...
    def doubanfm_playlist_url(self, channel_id=-10):
        data = {
            'channel': channel_id,
            'kbps': '192',
    ...
    def doubanfm_retrieve(self, kind, uri, cache_entry):
    ...
    def container_selection_changed_cb(self, selection):
        ...
        if self.doubanfm_cache_exists(cache_entry):
            ...
            self.doubanfm_retrieve(itemtype, uri, cache_entry)
    ...
    def build_sc_menu(self):
        """Create a Menu on self.sc_button: Powered by: douban.fm"""
        menu = {}
        ...
        if entry is not None and \
            ...
            menu[url] = _("View '%(title)s' on douban.fm") % {
    ...
    def doubanfm_random_song(self):
        self.new_model()
        url = self.doubanfm_playlist_url(channel_id=-10)
        self.loader.get_url(url, self.random_song_cb)

    def do_selected(self):
        """
        This is a action that you can do when the source is
        activated at the first time
        """
    ...
    def setup(self):
        ...
        builder = Gtk.Builder()
        builder.add_from_file(rb.find_plugin_file(self.props.plugin,
                                                  "doubanfm.ui"))
        ...
        self.container_view.get_selection().connect('changed', self.container_selection_changed_cb)
        ...
        grid = builder.get_object("doubanfm-source")
